...
for area in ["CA1", "CA3"]:
    for _, _, files in os.walk(rf"{base_folders[area]}"):
        tcl_filenames = [file for file in files if "all_cells_" in file]
        for tcl_filename in tqdm.tqdm(tcl_filenames):
...
plt.tight_layout()
plt.savefig(f"{output_root}/event_rate_allCells.pdf")
plt.savefig(f"{output_root}/event_rate_allCells.svg")
plt.close()
...
plt.tight_layout()
plt.savefig(f"{output_root}/event_rates_by_session_allCells.pdf")
plt.close()
