...
class CellDepth:
    def __init__(self, base_dir, session, imaging_logfile_name, show_rois=True):
        ...
        imaging_logfile_path = f"{self.base_dir}/{self.session}/{imaging_logfile_name}"
    ...
    def load_data(self):
        suite2p_folder = f"{self.base_dir}/{self.session}"
    ...
    def select_lines(self):
        ...
        self.gui = clicker(ax, ["deep", "superficial", "deep orient."], markers=["o", "*", "x"])
    ...
    def calculate_distances(self, show_percentages=True):
        ...
        cells[:, 0] = self.x_center
        cells[:, 1] = self.y_center
        rot_line_deep = self.rotate_points(self.points_deep, eigenvectors)
        rot_line_sup = self.rotate_points(self.points_sup, eigenvectors)
        rot_cells = self.rotate_points(cells, eigenvectors)
        rot_flag = self.rotate_points(self.flag_point[0, :], eigenvectors)
        ...
        if show_percentages:
            for i in range(self.x_center.size):
                ...
                plt.text(rot_cells[i, 0]+offset, rot_cells[i, 1]+offset, f"{100*norm_distances[i]:.2f}%")
                pass
...
if __name__ == "__main__":
    ...
    cd = CellDepth(base_dir, session, imaging_logfile_name, output_root)
